...
# Bergen Bysykkel 2024: Exploratory and Predictive Analytics
...
Here:\
- $(y)$ : Number of bike rides (response variable) - $(\beta_0)$: Intercept term, representing the baseline value of $(y)$ when all predictors are zero [-]{.underline} $(\beta_1, \beta_2, \dots, \beta_k)$: Coefficients representing the effect of predictors on $(y)$ - $(x_1, x_2, \dots, x_k)$: Predictors (e.g., weekday, hour of the day) [-]{.underline} $(\epsilon)$: Residual error, capturing the variability in $(y)$ not explained by the predictors
